...
class data_loader:
    def __init__(self, path):
        ...
        for link_type, adj in self.links['data'].items():
            # src_type = str(self.links['meta'][link_type][0])
            # dst_type = str(self.links['meta'][link_type][1])
            ...
            # print("link_type: ", link_type, "min_src: ", row.min(), "min_dst: ", col.min(), "src_shift: ", src_shift, "dst_shift: ", dst_shift)
    ...
    def gen_file_for_evaluate(self, test_idx, label, mode='bi'):
...
def load_imdb(feat_type=0, random_state=None):
    ...
    #     print("split IMDB with random_state = {}".format(random_state))
    # train_indices, valid_indices = train_test_split(train_valid_indices, test_size=val_ratio, stratify=np_labels[train_valid_indices], random_state=random_state)
    ...
    features_dict = dl.nodes["attr"]
    return hg, target_node_type, feature_node_types, features, features_dict, labels, num_classes, train_indices, valid_indices, test_indices, \
               train_mask, valid_mask, test_mask


def load_freebase(feat_type=1, random_state=None):
    dl = data_loader('./datasets/Freebase')
    ...
    np_labels = labels.detach().cpu().numpy()
    if random_state is not None:
        print("split Freebase with random_state = {}".format(random_state))
    train_indices, valid_indices = train_test_split(train_valid_indices, test_size=val_ratio, stratify=np_labels[train_valid_indices], random_state=random_state)
    train_indices = np.sort(train_indices)
    valid_indices = np.sort(valid_indices)
...
def load_dblp(feat_type=0, random_state=None):
    prefix = './datasets/DBLP'
    dl = data_loader(prefix)
    ...
    np_labels = labels.detach().cpu().numpy()
    if random_state is not None:
        print("split DBLP with random_state = {}".format(random_state))
    train_indices, valid_indices = train_test_split(train_valid_indices, test_size=val_ratio, stratify=np_labels[train_valid_indices], random_state=random_state)
    train_indices = np.sort(train_indices)
    valid_indices = np.sort(valid_indices)
...
def load_hgb_acm(feat_type=0, random_state=None):
    dl = data_loader('./datasets/ACM')
    ...
    np_labels = labels.detach().cpu().numpy()
    if random_state is not None:
        print("split HGB_ACM with random_state = {}".format(random_state))
    train_indices, valid_indices = train_test_split(train_valid_indices, test_size=val_ratio, stratify=np_labels[train_valid_indices], random_state=random_state)
    train_indices = np.sort(train_indices)
    valid_indices = np.sort(valid_indices)
